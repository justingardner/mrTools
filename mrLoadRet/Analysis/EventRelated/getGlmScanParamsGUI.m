...
while keepAsking
  ...
  %       end
       ...
       taskVarParams = {...
           ...
           {'forceStimOnTR', scanParams{scanNum}.forceStimOnTR, 'type=checkbox','Forces stimulus times to coincide with TR onsets'},...
             };

          if strcmp(params.analysisVolume,'Subset box')
           taskVarParams{end+1} = {'subsetBox', scanParams{scanNum}.subsetBox, 'subset of voxels,  the form [X1 X2;Y1 Y2;Z1 Z2] (Zs are optional)'};
          elseif strcmp(params.analysisVolume,'Whole volume')
           taskVarParams{end+1} = {'subsetBox', scanParams{scanNum}.subsetBox,'editable=0', 'subset of voxels,  the form [X1 X2;Y1 Y2;Z1 Z2] (Zs are optional)'};
          end
       ...
       elseif strfind(stimfile{1}.filetype,'eventtimes')  && ~isfield(scanParams{scanNum},'stimDuration') && isfield(stimfile{1}.mylog,'stimdurations_s')
        ...
        taskVarParams{end+1} = {'stimDuration', scanParams{scanNum}.stimDuration, 'duration of stimulation/event (seconds, min=0.01s), a boxcar function that is convolved with hrf. If using deconvolution, should be equal to TR'};
        ...
        if strcmp(params.analysisVolume,'Subset box') && params.covCorrection
          subsetBox = eval(scanParams{scanNum}.subsetBox);
          if any(diff(subsetBox(1:2,:),1,2)<=params.covEstimationAreaSize)
            mrWarnDlg('(getScanParamsGUI) subset box size is too small for covariance estimation area size');
            if ~ useDefault
              keepAsking = 1;
            else
              scanParams = [];
        ...
        if ~isempty(scanParams{scanNum}.preprocess)
          [d, failure] = eventRelatedPreProcess(d,scanParams{scanNum}.preprocess);
          if failure
            mrWarnDlg(['(getScanParamsGUI) There was a problem running pre-processing function ' scanParams{scanNum}.preprocess]);
            if ~ useDefault
              keepAsking = 1;
            else
              scanParams = [];
