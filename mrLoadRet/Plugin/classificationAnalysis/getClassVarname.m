...
while keepAsking
    for iScan=params.scanNum
        ...
        if ~isfield(scanParams{iScan},'eventLength')
            scanParams{iScan}.eventLength = 1;
        end
        if ~isfield(scanParams{iScan},'hdLag')
            scanParams{iScan}.hdLag = 2;
        end
        if ~isfield(scanParams{iScan},'averageEvent')
            scanParams{iScan}.averageEvent = 1;
        end   
        ...
        paramsInfo = {...
            ...
            {'eventLength',scanParams{iScan}.eventLength,'Length of your stimulus in TRs'}...
            {'hdLag',scanParams{iScan}.hdLag,'Estimated hemodynamic lag in TRs'}...
            {'averageEvent',scanParams{iScan}.averageEvent,'type=checkbox','Whether to average across the TRs for each stimulus'}...
