...
paramsInfo{end+1} = {'algorithm',{'nelder-mead','levenberg-marquardt', 'nelder-mead-bnd'},'Which algorithm to use for optimization. Levenberg-marquardt seems to get stuck in local minimum, so the default is nelder-mead. However, levenberg-marquardt can set bounds for parameters, so may be better for when you are trying to fit the hdr along with the rf, since the hdr parameters can fly off to strange values.'};
...
paramsInfo{end+1} = {'splitData', true, 'type=checkbox', 'Check this to split the data into chunks'};
paramsInfo{end+1} = {'numSplits', 10, 'minmax=[0 inf]','incdec=[-1 1]', 'Number of chunks to split the data into', 'contingent=splitData'};
paramsInfo{end+1} = {'splitOnly', false, 'type=checkbox', 'Check this to save the splits and then quit without running the pRF right now'};
