...
prefParams = {{'site',site,'Where you are using this code'},...
    ...
    {'overlayRangeBehaviour',overlayRangeBehaviour,'Behaviour of the overlay ''range'' field. In the ''Classic'' behaviour, setting the overlay range also sets the color scale range (if overlayCtype is set to ''Normal''). In the ''New'' behaviour, the ''range'' value only sets the overlay sliders range and the color range can be set separately using the ''colorRange'' field.'},...
    {'baseNaNsColor',baseNaNsColor,'How NaNs in the base anatomy should be displayed.'},...
...
if ~isempty(prefParams)
  ...
  mrSetPref('overlayRangeBehaviour',prefParams.overlayRangeBehaviour);
  mrSetPref('baseNaNsColor',prefParams.baseNaNsColor);
