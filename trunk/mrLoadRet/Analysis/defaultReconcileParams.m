...
if iscell(params)
  for i = 1:length(params)
    if ~isempty(params{i})
      defaultReconcileParams(groupName,params{i},data);
    end
  end
  return
end
...
if ~ieNotDefined('scanNums') && ~ieNotDefined('groupNum')
  ...
  if ~isfield(params,'tseriesFile')
	...
	disp(sprintf('(defaultReconcileParams) Scan %i has filename %s which does not match previous one %s',scanNums(iscan),viewGet([],'tseriesFile',scanNums(iscan),groupNum),params.tseriesFile{iscan}));
      end
    end
  end  
end
