...
%
function [view, params] = eventRelatedROIClassification(view,params,varargin)
...
%             end
        ...
        if params.sigTest
            if params.Linear
                linearP{scanNum}{r}=binomTest(sum(linearLab{scanNum}{r}==lab),length(lab),1/length(params.EVnames),'Greater');
            end
            if params.diagLinear
                diagP{scanNum}{r}=binomTest(sum(diagLab{scanNum}{r}==lab),length(lab),1/length(params.EVnames),'Greater');
            end
            if params.SVM
                svmP{scanNum}{r}=binomTest(sum(svmLab{scanNum}{r}==lab),length(lab),1/length(params.EVnames),'Greater');
            end
        end
