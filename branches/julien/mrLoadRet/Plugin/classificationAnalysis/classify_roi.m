function out = classify_roi(view,d,params,roi_n,select_vox)
...
[xxx r_idx] = loadScanRois(view,params.scanNum,roi_n);
if select_vox
...
for i = 1:size(xxx.coords,1)
xxx.sort_overlay(i)=overlay.data{1}(xxx.coords(i,1),xxx.coords(i,2),xxx.coords(i,3));
end
...
% end
% 
% % the co and ph values are loaded in from mat files, comment this out if
% % you don't require this
% % load co.mat
% % load ph.mat
% 
% d.roi{1} = loadROITSeries(view,roi{1});
% roin = 0;
%   for voxnum = 1:size(d.roi{roinum}.scanCoords,2)
%     x = d.roi{roinum}.scanCoords(1,voxnum);
%     y = d.roi{roinum}.scanCoords(2,voxnum);
%     s = d.roi{roinum}.scanCoords(3,voxnum);
%     %This section finds the relevant r2, co and ph values for each roi
%     %voxel. comment out the irrelevant parts.
%     rs(voxnum) =  d.r2(x,y,s);
% %     co_vox(voxnum) = co(x,y,s);
% %     ph_vox(voxnum) = ph(x,y,s);
%   end
% 
% % this part sorts the r2 and co values, as well as create a random order
% % for the voxels.
% [r2_sort r2_sortindex] = sort(rs,'descend');
% % [co_sort co_sortidx]=sort(co_vox,'descend');
% % rand_s = randperm(length(co_vox));
% % [rand_s_sort rand_s_index] = sort(rand_s,'descend');
% 
% 
% % the next section orders the roi voxels based on the chosen parameter,
% % comment out the irrelevant options.
% 
% % roi_t = d.roi{1}.tSeries(r2_sortindex,:);
% roi_t = d.roi{1}.tSeries(r2_sortindex,:);
% % % roi_t = d.roi{1}.tSeries(rand_s_index,:);
% % roi_ph = ph_vox(co_sortidx);
% 
% 
% % this part demeans the voxels, may be completely irrelevant
% % cm = mean(roi_t,2);
% % cm = repmat(cm,1,size(roi_t,2));
% % roi_t = 100*((roi_t-cm)./cm);

% cc = viewGet(view,'concatinfo');
% n=2;
% roi_t_n=roi_t;
% lab = [];
% for i=1:length(d.stimvol)
...
elseif params.eventLength>1 %create instance from each TR in the stim duration
    for i=1:size(idx,2)
        ...
        l_(idx(i):idx(i)+params.eventLength+params.hdLag-1)=repmat(lab(idx(i)),1,params.eventLength);
        r_(idx(i):idx(i)+params.eventLength+params.hdLag-1)=repmat(run(idx(i)),1,params.eventLength);
    end
    xxx.data=m_(l_>0);
    lab = l_(l_>0);
    run=r_(l_>0);
...
for r = 1:size(r_idx,2)
    ...
    if select_vox
       [sort_ patt_sort] =sort(xxx.sort_overlay(r_idx{r}),'descend');
       patt_sort=patt_sort(1:select_vox);
    elseif (select_vox > size(xxx.coords,1)) | (select_vox==0)
        patt_sort=1:size(xxx.coords,1);
    end
    
    for i=1:size(cc.runTransition,1)
    
    acc(i)=mean(lab(run==i)'==classify(patt(patt_sort,run==i)',patt(patt_sort,run~=i)',lab(run~=i),'diagLinear'));
    svm_acc(i)=mean(lab(run==i)'==classifyWithSvm(patt(patt_sort,run==i),patt(patt_sort,run~=i),lab(run~=i)));    
    end

m_acc{r}=mean(acc)
m_sacc{r}=mean(svm_acc)
end
...
for s=1:1000
    ...
    for r = 1:size(r_idx,2)
        for i=1:size(cc.runTransition,1)
        acc(i)=mean(s_lab(run==i)'==classify(xxx.data(r_idx{r},run==i)',xxx.data(r_idx{r},run~=i)',s_lab(run~=i),'diagLinear'));
        end
